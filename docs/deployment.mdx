...
4. (`https://dev.azure.com/ORG_NAME/REPO_NAME/_build` のように見える) プロジェクト・ページで、以下のテキストで新しいパイプラインを作成します。また、edit をクリックし、新しく生成したトークンをその値とする、`GH_TOKEN` という名前の新しい環境変数を作成し、次に `GH_EMAIL` (あなたのメールアドレス) と `GH_NAME` (GitHub ユーザー名) を作成します。必ずシークレットにしてください。あるいは、リポジトリのルートに `azure-pipelines.yml` というファイルを追加できます。
...
5. リポジトリをクリックし、activate repository をクリックし、`git_deploy_private_key` というシークレットに、先ほど生成した秘密鍵の値を追加します。
