...
---
 -->
---
description: Docusaurus は、簡単にインストールでき、すばやく Web サイトを立ち上げることができるように設計されています。
slug: /
---
...
⚡️ Docusaurus will help you ship a **beautiful documentation site in no time**.
 -->
⚡️ Docusaurus を使えば、**美しいドキュメントサイトをすぐに** 作成できます。
...
🧐 Docusaurus is a **static-site generator**. It builds a **single-page application** with fast client-side navigation, leveraging the full power of **React** to make your site interactive. It provides out-of-the-box **documentation features** but can be used to create **any kind of site** (personal website, product, blog, marketing landing pages, etc).
 -->
🧐 Docusaurus は、**静的サイトジェネレータ** です。高速な クライアントサイドのナビゲーションを備えた **シングルページ・アプリケーション** を構築し、**React** のフルパワーを活用してインタラクティブなサイトを作成します。すぐに使える **ドキュメント機能** を備えていますが、**あらゆる種類のサイト** (個人の Web サイト、製品、ブログ、マーケティングのランディングページなど) の作成に使用できます。
...
Docusaurus v2+ has been a total rewrite from Docusaurus v1, taking advantage of a completely modernized toolchain. After [v2's official release](https://docusaurus.io/blog/2022/08/01/announcing-docusaurus-2.0), we highly encourage you to **use Docusaurus v2+ over Docusaurus v1**, as Docusaurus v1 has been deprecated.
 -->
Docusaurus v2以降は、Docusaurus v1から全面的に書き直され、完全に近代化されたツールチェーンを利用しています。[v2の正式リリース](https://docusaurus.io/blog/2022/08/01/announcing-docusaurus-2.0)後、Docusaurus v1は廃止されたので、**Docusaurus v1よりも Docusaurus v2以降を使用する** ことを強くおすすめします。
...
- :white_check_mark: Internet Explorer 11のサポートは必要ない
...
- :x: Internet Explorer 11のサポートが必要 (…ですか ? Internet Explorer は[すでにサポートが終了](https://docs.microsoft.com/en-us/lifecycle/products/internet-explorer-11)しており、公式にはサポートされていません)

<!-- 
For existing v1 users that are seeking to upgrade to v2+, you can follow our [migration guides](./migration/index.mdx).
 -->
既存の v1ユーザーで v2以降へのアップグレードをお考えの方は、[移行ガイド](./migration/index.mdx)をご覧ください。
...
- ✂️ **開発者の体験**:
  - 今すぐドキュメントを書き始める
  - ユニバーサルなコンフィギュレーション・エントリーポイントにより、コントリビューターがより保守し易くなった
...
Docusaurus v2+ is born to be compassionately accessible to all your users, and lightning-fast.
 -->
Docusaurus v2以降は、すべてのユーザーが思いやりを持ってアクセスでき、かつ高速に動作するように設計されています。
...
- 🦖 **Accessible**. Attention to accessibility, making your site equally accessible to all users.
 -->
- ⚡️ **光速**。Docusaurus v2以降は、[PRPL パターン](https://developers.google.com/web/fundamentals/performance/prpl-pattern/)に従っているため、コンテンツの読み込みが非常に高速です。
...
- **No vendor lock-in**. Users are not required to use the default plugins or CSS, although they are highly encouraged to. Certain core infrastructures like React Loadable and React Router cannot be swapped because we do default performance optimization on them, but not higher-level ones. Choice of Markdown engines, CSS frameworks, CSS methodology, and other architectures will be entirely up to users.
 -->
- **覚えることが少ない**。Docusaurus は API が非常に小さいので、学習しやすく使いやすいはずです。たとえ書くコードや書く時間が増えたとしても、ほとんどのことはユーザーによって実現可能です。間違った抽象化をされているより、抽象化されていないほうがマシですし、ユーザーが間違った抽象化された部分をハックする必要はありません。必須の話 — [最小 API 表面積](https://www.youtube.com/watch?v=4anAwXYqLG8)。
- **直感的**。Docusaurus プロジェクトのプロジェクト・ディレクトリを見たり、新しい機能を追加したりするときに、ユーザーが圧倒されるような感じがしないこと。直感的で、使い慣れたアプローチで簡単に構築できること。
- **レイヤ構造アーキテクチャー**。スタック (コンテンツ/テーマ/スタイル) の各レイヤ間の懸念事項の分離は、明確で、よく抽象化され、モジュール化されていること。
- **賢明なデフォルト**。一般的でよく使われるパフォーマンスの最適化とコンフィギュレーションはユーザーのために行われるが、ユーザーにはそれを上書きするオプションが与えられている。
- **ベンダーロックインなし**。ユーザーはデフォルトのプラグインや CSS を使う必要はないが、使うことを強く推奨する。React Loadable や React Router のような特定のコア・インフラは交換できない。なぜなら、私たちはデフォルトでパフォーマンスの最適化を行っているが、より高いレベルのものは行っていないからだ。Markdown エンジン、CSS フレームワーク、CSS 方法論、その他のアーキテクチャーの選択は、完全にユーザー次第だ。
...
Across all static site generators, Docusaurus has a unique focus on documentation sites and has many out-of-the-box features.
 -->
すべての静的サイトジェネレータの中で、Docusaurus はドキュメントサイトに独特な重点を置いており、すぐに使える多くの機能を備えています。
...
[Gatsby](https://www.gatsbyjs.com/) is packed with a lot of features, has a rich ecosystem of plugins, and is capable of doing everything that Docusaurus does. Naturally, that comes at a cost of a higher learning curve. Gatsby does many things well and is suitable for building many types of websites. On the other hand, Docusaurus tries to do one thing super well - be the best tool for writing and publishing content.
 -->
[Gatsby](https://www.gatsbyjs.com/)には、多くの機能が詰め込まれており、豊富なプラグインのエコシステムがあり、Docusaurus ができることはすべてできます。当然ながら、学習曲線のコストも、そのぶん高くなります。Gatsby は多くのことをうまくこなし、多くのタイプの Web サイトを構築するのに適しています。一方、Docusaurus は、一つのことを非常にうまくやろうとしています - それは、コンテンツを書いて公開するための最高のツールであることです。

<!-- 
GraphQL is also pretty core to Gatsby, although you don't necessarily need GraphQL to build a Gatsby site. In most cases when building static websites, you won't need the flexibility that GraphQL provides.
 -->
GraphQL も Gatsby のかなりコアな部分ですが、Gatsby サイトを構築するのに、GraphQL が必ずしも必要なわけではありません。静的な Web サイトを構築するほとんどの場合、GraphQL が提供する柔軟性は必要ないでしょう。

<!-- 
Many aspects of Docusaurus v2+ were inspired by the best things about Gatsby and it's a great alternative.
 -->
Docusaurus v2以降の多くの部分は、Gatsby の良いところからインスパイアされたもので、すばらしい代替品です。
...
[Nextra](https://github.com/shuding/nextra) is an opinionated static site generator built on top of Next.js. It is currently less featured than Docusaurus.
 -->
[Nextra](https://github.com/shuding/nextra) は、Next.js の上に構築された静的サイトジェネレータです。現在のところ、Docusaurus よりも機能が少ないです。
...
[MkDocs](https://www.mkdocs.org/) is a popular Python static site generator with value propositions similar to Docusaurus.
 -->
[MkDocs](https://www.mkdocs.org/) は、Docusaurus に似た価値を提案する、人気の Python 静的サイトジェネレータです。
...
[Docsify](https://docsify.js.org/) makes it easy to create a documentation website, but is not a static-site generator and is not SEO friendly.
 -->
[Docsify](https://docsify.js.org/) は、ドキュメント Web サイトを簡単に作成できますが、静的サイトジェネレータではなく、SEO フレンドリーではありません。
...
[Jekyll](https://github.com/jekyll/jekyll) is one of the most mature static site generators around and has been a great tool to use — in fact, before Docusaurus, most of Facebook's Open Source websites are/were built on Jekyll! It is extremely simple to get started. We want to bring a similar developer experience as building a static site with Jekyll.
 -->
[Jekyll](https://github.com/jekyll/jekyll) は、最も成熟した静的サイトジェネレータの一つであり、すばらしいツールです — 実際、Docusaurus の前に、Facebook のオープンソースの Web サイトのほとんどは、Jekyll で構築されています/いました ! Jekyll は、非常に簡単に使い始めることができます。私たちは、Jekyll で静的サイトを構築するような開発者体験をもたらすことを望んでいます。
...
In comparison with statically generated HTML and interactivity added using `<script />` tags, Docusaurus sites are React apps. Using modern JavaScript ecosystem tooling, we hope to set new standards on doc sites' performance, asset building pipeline and optimizations, and ease to set up.
 -->
静的に生成された HTML や、`<script />` タグを使用して追加されたインタラクティブ性と比較すると、Docusaurus サイトは React アプリケーションです。最新の JavaScript エコシステム・ツールを使用することで、ドキュメント・サイトのパフォーマンス、アセット構築パイプライン、最適化、セットアップの容易さについて、新しい基準を設定したいと考えています。
...
If you find issues with the documentation or have suggestions on how to improve the documentation or the project in general, please [file an issue](https://github.com/facebook/docusaurus) for us, or send a tweet mentioning the [@docusaurus](https://x.com/docusaurus) X account.
 -->
もしドキュメントに課題を見つけたり、ドキュメントやプロジェクト全般を改善するための提案がある場合は、[issue を報告](https://github.com/facebook/docusaurus)していただくか、X アカウント [@docusaurus](https://x.com/docusaurus) 宛にメンションをツイートしてください。
